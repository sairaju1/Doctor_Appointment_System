...
const MyAppointments = () => {
  ...
  const cancelAppointment = async(appointmentId)=>{
    try{
      ...
      const {data}= await axios.post(backendUrl + '/api/user/cancel-appointment', {appointmentId},{headers: { Authorization: token }})
      if(data.success){
    ...
    const options = {
      ...
      handler:async(response)=>{
        ...
        try{
          ...
          const {data} = await axios.post(backendUrl+'/api/user/verifyRazorpay',response,{headers: { Authorization: token }})
          if(data.success){
            getUserAppointments()
  ...
  return (
    <div>
      ...
      <div>
        {appointments.length === 0 && <p className="text-center text-gray-500 mt-6">No appointments found.</p>}
        {appointments.map((item, index) => (
          <div
            className="grid grid-cols-[1fr_2fr] gap-4 sm:flex gap-6 py-2 border-b"
            key={index}
          >
            <div>
              <img
